...
(async () => {
	try {
		await replace({
			...
			from: /github.com\/ptkdev-boilerplate\/svelte-kit-ssr-boilerplate/g,
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /github.com\/ptkdev\/svelte-kit-ssr-boilerplate/g,
			to: setup.github_full_repository_url,
		});
	} catch (error) {
		console.error("Error occurred:", error);
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /@ptkdev\/svelte-kit-ssr-boilerplate/g,
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /Svelte server-side-rendering built with SEO in mind. Start your project with this friendly svelte kit boilerplate/g,
	...
	try {
		await replace({
			...
			from: /"projectName": "ptkdev-boilerplate\/svelte-kit-ssr-boilerplate"/g,
	...
	try {
		await replace({
			files: ["**/*", ".*", "**/.*"],
			ignore: ["node_modules/**/*", "setup.json", "package-lock.json", "scripts/setup.ts", ".all-contributorsrc"],
			from: /svelte-kit-ssr-boilerplate/g,
